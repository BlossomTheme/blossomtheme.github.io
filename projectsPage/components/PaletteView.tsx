  ...
  return (
    <div className="space-y-16 pb-24">
      ...
      {colorGroups.map((group, groupIdx) => (
        <div key={groupIdx} className="space-y-6">
          <div className="flex items-end gap-4 border-b-8 border-black pb-2 mb-8">
             <div className={`${group.accent} w-10 h-10 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]`}></div>
             <h3 className="manga-font text-4xl md:text-5xl text-white uppercase">{group.title}</h3>
             <span className="japanese-font text-xl text-gray-500 font-black mb-1 opacity-50">{group.jpTitle}</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {group.colors.map((color, colorIdx) => (
              ...
              >
                <div 
                  className="h-28 md:h-36 border-[4px] border-black transition-all group-hover:-translate-y-2 group-hover:translate-x-1 group-active:translate-y-0 group-active:translate-x-0 relative overflow-hidden"
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-black/60 flex items-center justify-center transition-opacity duration-200 ${copied === color.hex ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <div className="bg-white text-black px-3 py-1 border-2 border-black font-black text-[10px] uppercase rotate-[-5deg]">
                ...
                <div className="mt-4 space-y-1">
                   <div className="text-[10px] font-black uppercase text-purple-400 tracking-tighter opacity-70">
                     {color.hex}
                   </div>
                   <div className="text-xs md:text-sm font-black text-white leading-none truncate pr-2">
                     {color.name}
                   </div>
                </div>
              </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
         <div className="bg-white text-black border-4 border-black p-8 relative overflow-hidden shadow-[12px_12px_0px_0px_rgba(168,85,247,1)]">
            <div className="absolute -bottom-10 -right-10 halftone w-40 h-40"></div>
            <h4 className="manga-font text-4xl mb-4 flex items-center gap-2"><Palette className="text-purple-600" /> USAGE GUIDELINES</h4>
         ...
         <div className="bg-black border-4 border-white p-8 flex flex-col justify-center items-center text-center group">
            <div className="text-6xl mb-4 transition-transform group-hover:scale-110">🎨</div>
            <h4 className="manga-font text-3xl text-white mb-2 italic">MISSING A COLOR?</h4>
            <p className="text-gray-400 font-bold text-sm max-w-xs mb-6 uppercase tracking-widest">Contribute to the spectrum on our official GitHub</p>
            <a 
               href="https://github.com/BlossomTheme" 
               target="_blank" 
               className="bg-purple-600 text-white border-4 border-white px-8 py-3 manga-font text-2xl hover:bg-white hover:text-black transition-all shadow-[6px_6px_0px_0px_rgba(255,255,255,1)]"
            >
               PROPOSE NEW INK
            </a>
         </div>
