...
const ThemeGallery: React.FC = () => {
  ...
  return (
    <div className="w-full space-y-12">
      {/* Hero Section Banner */}
      <div className="relative border-4 border-black bg-purple-600 p-6 sm:p-8 md:p-12 overflow-hidden shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
        <div className="absolute inset-0 halftone"></div>
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="max-w-2xl">
            <h2 className={`${MANGA_STYLES.heading} text-white text-3xl sm:text-5xl md:text-7xl mb-2 italic`}>OUR WORK</h2>
            <p className="japanese-font font-bold text-pink-200 text-sm sm:text-base md:text-lg italic bg-black/30 inline-block px-2 sm:px-3 py-1 uppercase tracking-widest">"EVERY LINE OF CODE IS A STROKE OF INK"</p>
          </div>
          <div className="bg-white text-black p-3 sm:p-4 border-4 border-black -rotate-2 hidden md:block shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
             <p className="font-bold uppercase tracking-tighter text-xs">Exploration Phase</p>
             <p className="manga-font text-2xl sm:text-3xl leading-none">ACT 02</p>
          </div>
        </div>
      </div>
        ...
        {themes.map((theme, idx) => (
          ...
          >
            ...
            <div className="h-48 overflow-hidden border-b-4 border-black relative">
              ...
              />
              <div className="absolute top-2 right-2 bg-pink-500 border-2 border-black px-2 py-0.5 text-[10px] sm:text-xs font-bold text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                {theme.software.toUpperCase()}
              </div>
            </div>

            {/* Info */}
            <div className="p-3 sm:p-4 flex-grow flex flex-col justify-between bg-white group-hover:bg-purple-50 transition-colors">
              <h3 className="manga-font text-lg sm:text-2xl text-black leading-tight group-hover:text-purple-600 transition-colors">{theme.name}</h3>
              <div className="flex justify-between items-end">
                <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400">VOL. {idx + 1}</span>
                <span className="bg-black text-white px-2 sm:px-3 py-1 text-[8px] sm:text-xs font-bold uppercase transform -skew-x-12">View Details</span>
              </div>
            </div>
